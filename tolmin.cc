...
{
    /* Format strings */
    static char fmt_1010[] = "(/5x,\002ERROR RETURN FROM GETMIN BECAUSE A CO\
NDITION\002,\002 ON N, M OR MEQ IS VIOLATED\002)";
    static char fmt_1020[] = "(/5x,\002ERROR RETURN FROM GETMIN BECAUSE A LO\
WER\002,\002 BOUND EXCEEDS AN UPPER BOUND\002)";
    static char fmt_1030[] = "(/5x,\002ERROR RETURN FROM GETMIN BECAUSE TH\
E\002,\002 EQUALITY CONSTRAINTS ARE INCONSISTENT\002)";
    static char fmt_1040[] = "(/5x,\002ERROR RETURN FROM GETMIN BECAUSE TH\
E\002,\002 EQUALITIES AND BOUNDS ARE INCONSISTENT\002)";
    static char fmt_1050[] = "(/5x,\002ERROR RETURN FROM GETMIN BECAUSE TH\
E\002,\002 CONSTRAINTS ARE INCONSISTENT\002)";
    static char fmt_1060[] = "(/5x,\002GETMIN FINDS THAT THE VARIABLES AR\
E\002,\002 DETERMINED BY THE EQUALITY CONSTRAINTS\002)";
...
L40:
    ...
    lsrch_(n, &x[1], &g[1], &d__[1], &xs[1], &gs[1], relacc, &stepcb, &ddotg, 
	    f, &step, nfvals, nfmax, &bres[1],Info);
    if (step == (float)0.) {
	*info = 3;
	sum = (float)0.;
	i__1 = *n;
		...
		MinInfo &info)
{
	...
	*f=info.p->funmin(Xa);
	if(isnan(*f) || isinf(*f)) *f=1e+10;
	info.p->granal(Xa,Ga);
	{
		if(*f<=oldmin)
		{
		 //for(int i=0;i<Xa.size();i++) printf("%lf ",Xa[i]);
		 //printf("\n");
//		 printf(" iter%d value=%lf\n",totcal_1.itnocs,*f);
	/*	Neural *nn=(Neural*)info.p;
		double viol=nn->countViolate(20.0);
		if(viol<=oldViol) oldViol=viol;
		else totcal_1.itnocs=10000;
*/
		//cout<<"ITER="<<totcal_1.itnocs<<" VALUE="<<*f<<endl;
		 oldmin=*f;
		}
	}
	for(int i=0;i<*n;i++)
	...
	cgrad)
{
    ...
    /* Local variables */
    static double temp;
    static integer i__, j, k, ii, jm, iz;
    extern /* Subroutine */ int delcon_(integer *, integer *, double *, 
	    integer *, integer *, integer *, double *, double *, 
